...
{
	if (!h || sym_type < 0 || sym_type >= SYM_NUM)
		return -1;
...
const char *api_get_symbol_name(policy_handle_t *h, int sym_type, int value)
{
	...
	if (sym_type < 0 || sym_type >= SYM_NUM)
		return NULL;
	// Indices in the names table are 0-based.
	if (value <= 0 || (uint32_t)value > db->symtab[sym_type].nprim)
		return NULL;
...
int api_is_type_attribute(policy_handle_t *h, int type_val)
{
	if (!h)
		return -1;
	policydb_t *db = &((struct sepol_policydb *)(h->db))->p;
	if (type_val <= 0 || (uint32_t)type_val > db->p_types.nprim)
		return -1;
...
int api_get_boolean_state(policy_handle_t *h, int bool_val)
{
	if (!h)
		return -1;
	policydb_t *db = &((struct sepol_policydb *)(h->db))->p;
	if (bool_val <= 0 || (uint32_t)bool_val > db->p_bools.nprim)
		return -1;
...
static int collect_rules(avtab_key_t *k, avtab_datum_t *d, void *ptr)
{
	...
	if (!(k->specified & state->specified_mask))
		return 0;
	...
	if (state->query && strlen(state->query) > 0) {
		const char *src_name = api_get_symbol_name(
			state->handle, SYM_TYPES, k->source_type);
		const char *tgt_name = api_get_symbol_name(
			state->handle, SYM_TYPES, k->target_type);
		const char *cls_name = api_get_symbol_name(
			state->handle, SYM_CLASSES, k->target_class);
		...
		if (state->is_regex && state->regex_compiled) {
			...
			if (src_name && regexec(state->regex_compiled, src_name,
						0, NULL, 0) == 0)
				match = 1;
			if (!match && tgt_name &&
			    regexec(state->regex_compiled, tgt_name, 0, NULL,
				    0) == 0)
				match = 1;
			if (!match && cls_name &&
			    regexec(state->regex_compiled, cls_name, 0, NULL,
				    0) == 0)
				match = 1;
		} else {
			// Standard substring matching (default).
			if (src_name && strstr(src_name, state->query))
				match = 1;
			if (!match && tgt_name &&
			    strstr(tgt_name, state->query))
				match = 1;
			if (!match && cls_name &&
			    strstr(cls_name, state->query))
				match = 1;
		}
 ...
 */
EMSCRIPTEN_KEEPALIVE
int api_get_rules(policy_handle_t *h, rule_info_t *out_rules, int max_rules,
		  const char *query, int is_regex, uint16_t specified_mask)
{
	if (!h)
		return -1;
	policydb_t *db = &((struct sepol_policydb *)(h->db))->p;
	...
	struct rule_collect_state state = { h,	       out_rules,     0,
					    max_rules, query,	      is_regex,
					    regex_ptr, specified_mask };
...
char *api_get_permissions(policy_handle_t *h, int class_val, uint32_t data)
{
	if (!h)
		return NULL;
	policydb_t *db = &((struct sepol_policydb *)(h->db))->p;
	if (class_val <= 0 || (uint32_t)class_val > db->p_classes.nprim)
		return NULL;
